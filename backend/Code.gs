...
const GLOBAL_SHEETS_CONFIG = {
    // ผู้ใช้งานระบบ (USERS & ROLES)
    'Users': ['id', 'employeeId', 'name', 'role', 'permissions', 'email', 'avatar', 'isDev', 'status', 'createdAt', 'updatedAt'],
    
    // CERTIFICATES COLLECTIONS
    'Certificates_Master': ['id', 'title', 'documentNo', 'issueDate', 'expiryDate', 'status', 'summary', 'fileUrl', 'tags', 'addedBy', 'createdAt', 'updatedAt'],
    'Certificates_FDA': ['id', 'title', 'documentNo', 'issueDate', 'expiryDate', 'status', 'summary', 'fileUrl', 'tags', 'addedBy', 'createdAt', 'updatedAt'],
    'Certificates_Halal': ['id', 'title', 'documentNo', 'issueDate', 'expiryDate', 'status', 'summary', 'fileUrl', 'tags', 'addedBy', 'createdAt', 'updatedAt'],
    'Certificates_ISO': ['id', 'title', 'documentNo', 'issueDate', 'expiryDate', 'status', 'summary', 'fileUrl', 'tags', 'addedBy', 'createdAt', 'updatedAt'],
    'Certificates_Other': ['id', 'title', 'documentNo', 'issueDate', 'expiryDate', 'status', 'summary', 'fileUrl', 'tags', 'addedBy', 'createdAt', 'updatedAt'],
    
    // COMPLIANCE & ACTIONS (การปฏิบัติตามมาตรฐาน)
    ...
    'Evidence_Log': ['id', 'title', 'description', 'relatedCert', 'fileUrl', 'submittedBy', 'status', 'createdAt', 'updatedAt'],
    'Task_ActionPlans': ['id', 'taskName', 'description', 'assignedTo', 'priority', 'status', 'dueDate', 'relatedCert', 'progress', 'createdAt', 'updatedAt'],

    // DISPUTES & COMPLAINTS (เรื่องร้องเรียน/ข้อพิพาท)
...
function setupDatabase() {
  ...
  if (userSheet && userSheet.getLastRow() === 1) {
    userSheet.appendRow(['1', 'ADMIN001', 'System Admin', 'Admin', '{"canCreate":true,"canEdit":true,"canApprove":true,"canVerify":true}', 'admin@smartlaw.com', '', 'true', 'Active', new Date().toISOString(), new Date().toISOString()]);
  }
...
function doPost(e) {
  try {
    ...
    // [OPTIONAL] Basic Security: Check API Key
    // const EXPECTED_API_KEY = PropertiesService.getScriptProperties().getProperty('API_KEY');
    // if (EXPECTED_API_KEY && apiKey !== EXPECTED_API_KEY && action !== 'login') {
    ...
    if (!ss) {
      return createResponse("error", "Spreadsheet connection failed. Please ensure SPREADSHEET_ID is set in Script Properties or the script is bound to a Spreadsheet.");
    }
    
    // Auto-Provisioning: สร้างชีตใหม่และหัวตารางถ้าไม่พบ
    ...
    let result;
    // Routing action ตามคำสั่งที่ส่งมาจาก Frontend
    switch (action) {
      case 'read':
        result = readData(sheet, params);
        break;
      case 'lookup':
        result = lookupData(sheet, params); 
        break;
      case 'login':
      ...
      case 'update':
      case 'delete':
        // ใช้ LockService เพื่อป้องกันปัญหา Concurrency 
        const lock = LockService.getScriptLock();
        if (!lock.tryLock(30000)) { // รอสูงสุด 30 วินาที
          return createResponse("error", "Lock timeout: Server is busy, please try again.");
        }
        try {
          if (action === 'write') result = writeData(sheet, data);
          else if (action === 'update') result = updateData(sheet, data);
          else if (action === 'delete') result = deleteData(sheet, data);
        } finally {
          lock.releaseLock();
        }
        break;
      default:
        result = createResponse("error", "Unknown action: " + action);
    }
    return result;
  } catch (err) {
...
function doGet(e) {
  // สำหรับเทส URL ในเบราว์เซอร์
  return createResponse("success", "SMART LAW API is active. Please use POST for data operations.");
}
...
function writeData(sheet, data) {
  ...
  return createResponse("success", "Data saved successfully (" + rows.length + " rows)");
}

function updateData(sheet, data) {
  if (!Array.isArray(data)) data = [data];
  if (data.length === 0) return createResponse("error", "No data provided for update");

  const range = sheet.getDataRange();
  const values = range.getValues();
  const headers = values[0];
  const idIndex = headers.indexOf('id');
  if (idIndex === -1) return createResponse("error", "'id' column not found for update");
  ...
  return createResponse("success", `Updated ${updatedCount} rows`);
}

function deleteData(sheet, data) {
  if (!Array.isArray(data)) data = [data];
  if (data.length === 0) return createResponse("error", "No data provided for delete");

  const range = sheet.getDataRange();
  const values = range.getValues();
  const headers = values[0];
  const idIndex = headers.indexOf('id');
  if (idIndex === -1) return createResponse("error", "'id' column not found for delete");
  ...
  return createResponse("success", `Deleted ${deletedCount} rows`);
}

function lookupData(sheet, params) {
  const dataList = params.data;
  const matchType = params.matchType || 'exact'; 
  
  if (!dataList || dataList.length === 0) return createResponse("error", "Lookup requires 'data' array containing search object");
  var criteria = dataList[0]; 
  var criteriaKeys = Object.keys(criteria);
  
  var rows = sheet.getDataRange().getValues();
  if (rows.length <= 1) return createResponse("success", "No data found", { items: [] });
...
function handleLogin(ss, credentials) {
  const userSheet = ss.getSheetByName("Users");
  if (!userSheet) return createResponse("error", "Users sheet not found");

  const values = userSheet.getDataRange().getValues();
  if (values.length <= 1) return createResponse("error", "No users found");
