...
import { 
  ...
  Eye, ChevronLeft, ChevronRight, HelpCircle, ChevronDown, MapPin, AlertTriangle, CheckCircle 
} from 'lucide-react';
...
const generateMockLogs = () => {
  ...
  const users = ['SOMCHAI PROCESS', 'SUDA ASSESSOR', 'HR SYSTEM Developer', 'SARAH SUPPORT', 'UNKNOWN_USER'];
        ...
        <div className="w-full flex-1 flex flex-col min-h-0">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3 shrink-0">
            ...
            <KpiCard 
              ...
              colorValue={THEME.primary} 
              desc="Active Identities" 
            />
          </div>

          <div className="bg-white rounded-2xl shadow-sm border border-[#eaeaec]/60 overflow-hidden flex flex-col animate-fadeIn font-sans">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full md:w-auto">
                <div className="relative shrink-0">
                  <button 
                    ...
                    className="flex items-center gap-3 bg-[#f8f9fa] px-4 py-2.5 rounded-xl border border-[#eaeaec] shadow-sm hover:border-[#b7a159] hover:bg-white transition-all min-w-[180px] cursor-pointer"
                  >
                  ...
                  {filterDropdownOpen && (
                    <>
                      ...
                      <div className="absolute top-[110%] left-0 w-full bg-white border border-[#eaeaec] shadow-2xl rounded-2xl p-2 z-50 animate-in fade-in slide-in-from-top-2 text-left">
                        {['All', 'Success', 'Failed', 'Warning'].map(status => (
                          <button 
                            ...
                            className={`w-full flex items-center p-3 rounded-xl transition-all cursor-pointer ${
                              statusFilter === status ? 'bg-[#f8f9fa] text-[#212c46] font-black' : 'hover:bg-[#f8f9fa] text-[#7a8b95] font-medium'
                            }`}
                          >
                ...
                <div className="relative flex-1 md:w-80">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-4 py-2.5 text-[12px] font-bold rounded-2xl border border-[#eaeaec] focus:outline-none focus:border-[#b7a159] bg-[#f8f9fa] focus:bg-white shadow-sm text-[#212c46] transition-all" 
                  />
                </div>
              </div>
              <div className="flex gap-3 shrink-0 w-full md:w-auto">
                <button className="w-full md:w-auto px-6 py-2.5 bg-gradient-to-r from-[#212c46] to-[#414757] text-white rounded-2xl text-[11px] font-black tracking-widest uppercase shadow-md hover:scale-105 transition-all flex items-center justify-center gap-2 border border-[#212c46] cursor-pointer">
            ...
            <div className="overflow-x-auto custom-scrollbar bg-[#f8f9fa]">
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#212c46] text-white sticky top-0 z-10 border-b-2 border-[#b7a159]">
                  <tr>
                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap font-mono">Date / Time</th>
                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap font-mono">User Identity</th>
                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap font-mono">IP Address</th>
                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] whitespace-nowrap font-mono">Module & Action</th>
                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] text-center whitespace-nowrap font-mono">Status</th>
                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[12px] text-center whitespace-nowrap font-mono">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#eaeaec] bg-white">
                  {paginatedLogs.length > 0 ? paginatedLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-[#f8f9fa] transition-colors group">
                      ...
                      <td className="py-3 px-6 text-left">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] font-bold text-[#7a8b95] mt-0.5 uppercase tracking-wider font-mono">{log.role}</span>
                        </div>
                      </td>
                      <td className="py-3 px-6 font-mono text-[12px] font-bold text-[#4d87a8] text-left">
                      ...
                      <td className="py-3 px-6 text-center">
                        <span className={`px-3 py-1 rounded-full border font-black text-[11px] uppercase tracking-widest whitespace-nowrap font-mono ${getStatusStyle(log.status)}`}>
                          {log.status}
                        </span>
                      </td>
                      <td className="py-3 px-6 text-center">
                        <div className="flex justify-center items-center gap-[1px]">
                          <button 
                            ...
                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-[#eaeaec] text-[#4d87a8] hover:bg-[#d7d7d7] hover:border-[#4d87a8] transition-all active:scale-90 cursor-pointer" 
                            title="View Details"
                          >
                            <Eye size={14} />
                          </button>
                    ...
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-[#7a8b95] font-bold text-[12px] font-mono">
            ...
            <div className="px-8 py-3 bg-[#f8f9fa] border-t-[1.5px] border-slate-300 flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
              <div className="flex items-center gap-6 text-[11px] font-black text-[#7a8b95] uppercase tracking-widest font-mono">
                <div className="flex items-center gap-3">
                  <span>Display Rows:</span>
                  <select 
                    value={itemsPerPage} 
                    onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} 
                    className="bg-white border border-[#eaeaec] rounded-lg px-3 py-1.5 outline-none font-black text-[#212c46] cursor-pointer shadow-sm focus:border-[#b7a159]"
                  >
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className={`w-9 h-9 border border-[#eaeaec] bg-white rounded-lg flex items-center justify-center transition-all ${
                    currentPage === 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-[#d7d7d7] text-[#4d87a8] shadow-sm active:scale-90 cursor-pointer'
                  }`}
                >
                  <ChevronLeft size={16} />
                </button>
                <div className="bg-white text-[#212c46] px-6 py-2 rounded-lg font-black text-[11px] min-w-[120px] text-center uppercase tracking-widest border border-[#eaeaec] shadow-sm font-mono">
                ...
                <button 
                  ...
                  className={`w-9 h-9 border border-[#eaeaec] bg-white rounded-lg flex items-center justify-center transition-all ${
                    currentPage === totalPages ? 'opacity-30 cursor-not-allowed' : 'hover:bg-[#d7d7d7] text-[#4d87a8] shadow-sm active:scale-90 cursor-pointer'
                  }`}
                >
