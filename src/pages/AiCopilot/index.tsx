...
const THEME = {
  ...
  coolGray: '#dbdad6',
  darkRed: '#8cb3b2',       // map custom names
  oliveGreen: '#93a052',
  sageGreen: '#b0b6ac',
  burgundy: '#bc6868',
  khaki: '#b9a781',
...
const suggestedPrompts = [
  { id: 'p1', title: 'Annual Leave Policy', text: 'สิทธิ์การลาพักร้อนประจำปี (Annual Leave Policy) ของพนักงานแต่ละระดับชั้นสามารถลาได้สูงสุดกี่วันและต้องล่วงหน้ากี่วัน?', icon: 'CalendarDays' },
  { id: 'p2', title: 'Provident Fund (PVD)', text: 'การหักและสมทบทุนส่วนของระบบทุนสำรองเลี้ยงชีพ (Provident Fund) มีเงื่อนไขการหักและการได้สิทธิ์คืน (Vesting) อย่างไร?', icon: 'Coins' },
  { id: 'p3', title: 'OPD / IPD Underwriter', text: 'สิทธิ์ค่ารักษาพยาบาลกรณีคนไข้นอก (OPD) และคนไข้ใน (IPD) ของประกันกลุ่มบริษัทมีวงเงินครอบคลุมอย่างไรบ้าง?', icon: 'HeartHandshake' },
  { id: 'p4', title: 'Labor & Maternity Leave', text: 'สรุปข้อกฎหมายเกี่ยวกับสิทธิ์การลาคลอดและการจ่ายค่าจ้างพนักงานตามกฎหมายแรงงานไทยล่าสุด', icon: 'Scale' }
];
...
export default function AiCopilot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: 1, role: 'ai', text: 'สวัสดีค่ะ! ฉันคือ T All BOT ผู้ช่วยอัจฉริยะส่วนตัวของคุณ วันนี้มีสิทธิ์สวัสดิการพนักงาน นโยบายบริษัทด้านการลา หรือเรื่องใบรับรองคุณภาพส่วนไหนเพิ่มความสอดคล้องที่คุณประสงค์อยากปรึกษาพิจารณาอีกไหมคะ?', timestamp: '08:00 AM' }
  ]);
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        if (data.needKey) {
            // Fallback mode
            let aiReply = "⚠️ ไม่พบ GEMINI_API_KEY ในการตั้งค่า (Settings > Secrets)\n\nระบบสลับเข้าสู่โหมดแนะนำข้อมูลสวัสดิการและนโยบายออฟไลน์ชั่วคราว:\n\n";
            if (text.includes('ลาพักร้อน') || text.includes('Annual Leave') || text.includes('p1')) {
              aiReply += "**ระเบียบการลาพักร้อนประจำปี (Annual Leave Policy)**:\n\n" +
                         "1. **อายุงาน 1-3 ปี**: ได้สิทธิ์วันลาพักผ่อน 10 วันทำการต่อปีปฏิทิน\n" +
                         "2. **อายุงาน 3-5 ปี**: ได้สิทธิ์เพิ่มขึ้นเป็น 12 วันทำการต่อปี\n" +
                         "3. **อายุงานมากกว่า 5 ปีขึ้นไป**: ได้รับสิทธิ์สูงสุด 15 วันทำการต่อปี\n\n" +
                         "*กำหนดส่งคำอนุมัติ: ต้องยื่นขอออนซิงค์ผ่านระบบอย่างน้อย 3 วันก่อนวันเดินทางจริงเพื่อให้อนุมัติราบรื่น*";
            } else if (text.includes('สำรองเลี้ยงชีพ') || text.includes('Provident') || text.includes('p2') || text.includes('PVD')) {
              aiReply += "**นโยบายกองทุนสำรองเลี้ยงชีพ (Provident Fund - PVD)**:\n\n" +
                         "1. **การสะสมพนักงาน**: หักจากเงินเดือนพื้นฐานได้ตั้งแต่ 2% ถึง 15% ตามความสมัครใจ\n" +
                         "2. **สิทธิ์เงินสมทบคณะกรรมการ**: บริษัทสมทบให้อัตราเดียวสูงสุดที่ 5% ของเงินพนักงาน\n" +
                         "3. **เกณฑ์สะสมสิทธิ์รับเงินสมทบเมื่อพ้นสภาพ (Vesting Schedule)**:\n" +
                         "   - อายุงานน้อยกว่า 3 ปี: ได้รับสิทธิ์รับเงินสมทบจากบริษัท 0%\n" +
                         "   - อายุงาน 3-5 ปี: ได้สิทธิ์รับเงินสมทบของบริษัทกึ่งหนึ่ง (50%)\n" +
                         "   - อายุงานมากกว่า 5 ปี: ได้สิทธิ์รับเงินสมทบบริษัทเต็มพอร์ต (100%)";
            } else if (text.includes('OPD') || text.includes('รักษาพยาบาล') || text.includes('p3') || text.includes('ประกันสุขภาพ')) {
              aiReply += "**บริการสิทธิประโยชน์ด้านการแพทย์และสุขภาพ (Health Insurance Benefits)**:\n\n" +
                         "1. **ผู้ป่วยนอก (OPD)**: ครอบคลุมสิทธิ์รักษาไม่เกิน 2,000 บาทต่อครั้ง จำกัดไม่เกิน 15 ครั้งต่อปีปฏิทิน (ชำระตรงผ่านประกันภัยกลุ่ม)\n" +
                         "2. **ผู้ป่วยใน (IPD)**: คุ้มครองกรณีรักษาตัวครองเตียงในโรงพยาบาลพันธมิตรรวมวงเงินสูงสุด 100,000 บาทต่ออุบัติการณ์\n" +
                         "3. **สวัสดิการส่งเสริมการพัฒนาความรู้**: งบสนับสนุนสอบใบประกาศวิชาชีพและวิชาการเสริมสร้างศักยภาพ 15,000 บาทต่อคน/ปีปฏิทิน";
            } else if (text.includes('ลาคลอด') || text.includes('Maternity') || text.includes('p4')) {
              aiReply += "**กฎหมายและสิทธิ์วันลาคลอก (Maternity Leave Regulations)**:\n\n" +
                         "1. พนักงานสตรีที่ครรภ์ได้รับสิทธิ์วันพักผ่อนและเตรียมคลอดได้สูงสุด 98 วันต่อการคลอดบุตรหนึ่งครั้ง\n" +
                         "2. บริษัทรับผิดชอบจ่ายเงินค่าเรียนและค่าจ้างเต็มจำนวนจำนวน 45 วันแรกของวันลาดังกล่าว\n" +
                         "3. อีก 45 วันทำการถัดไป พนักงานมีหน้าที่ยื่นยันเอกสารคำขอเบิกสิทธิ์เงินพัดและอำนาจชดเชยจากกองทุนประกันสังคม";
            } else if (text.includes('ISO 9001')) {
              ...
              aiReply += "กำลังประมวลผลข้อมูล... ระบบไม่พบคำตอบออฟไลน์ที่บันทึกไว้ตรงกับหัวข้อ กรุณาตั้งค่า API Key เพื่อเปิดใช้งานระบบ Real-Time Search Grounding ในการสแกนหาคำตอบภายนอกทันที";
            }
