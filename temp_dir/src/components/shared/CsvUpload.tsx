    ...
    <div className="space-y-6">
      ...
      {!file ? (
        <div 
          ...
          onDrop={handleDrop}
          className={`border-2 border-dashed rounded-[24px] p-12 flex flex-col items-center justify-center gap-4 transition-all cursor-pointer group ${
            isDragging 
              ? 'border-[#E3624A] bg-orange-50/50' 
              : 'border-slate-200 hover:border-[#E3624A] hover:bg-orange-50/30'
          }`}
        >
          <div className={`p-4 rounded-full transition-all ${
            isDragging 
              ? 'bg-[#E3624A] text-white scale-110' 
              : 'bg-slate-50 text-slate-400 group-hover:bg-[#E3624A] group-hover:text-white'
          }`}>
            <Upload size={32} />
          </div>
          <div className="text-center">
            <p className="text-sm font-black text-[#111f42] uppercase tracking-widest">
              {isDragging ? 'Drop file here' : 'Click to upload Excel / CSV'}
            </p>
            <p className="text-[10px] text-slate-400 uppercase tracking-widest mt-1">
              {isDragging ? 'Release to upload' : 'or drag and drop your file here'}
            </p>
          </div>
          <input 
            type="file" 
