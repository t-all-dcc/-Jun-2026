...
- **Page Container Spacing:** Every page must follow the base container structure: `<div className="flex flex-1 w-full flex-col animate-fadeIn bg-transparent space-y-4">`. **NEVER** add `pb-6` or any bottom padding to the main container.
- **Page Left/Right Margin Padding (Important):** To prevent headers and inner contents from sticking to the edges of the browser window, all pages must exact-match the spacing of the **User Permissions** page.
  - The **Page Header** div must have exactly: `h-14 px-8 flex flex-row items-center justify-between gap-4 z-20 shrink-0` (or similar flex settings, but MUST include padding `px-8`).
  - The **Content Section** directly following the header must be wrapped entirely inside a container matching: `<div className="w-full px-4 sm:px-8 flex-1 flex flex-col min-h-0">` which wraps the KPI metrics, tables, filters, lists, and pages contents.
